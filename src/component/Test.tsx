...
export const Test = (props: { children: ReactChild; seachbar?: true }) => {
  const Logout = () => {
    const { logout } = useUser();
    return (
      <button
        onClick={logout}
        className="bg-blue-500 ml-4 rounded-lg text-white py-3 px-8"
      >
        logout
      </button>
    );
                  ...
                  return (
                    <Link key={item.href} href={item.href}>
                      <a
                        className={`${
                          item.now && now
                        } w-full font-thin uppercase text-gray-500 dark:text-gray-200 flex items-center p-4 my-2 transition-colors duration-200 justify-start hover:text-blue-500s`}
                      >
